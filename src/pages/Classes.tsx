  ...
  const isClassPast = (classBatch: IClassBatch): boolean => {
    ...
    if (!firstSession || !firstSession.endDate) {
      ...
      if (firstSession?.startDate) {
        const sessionEndTimestamp = istDayEnd(firstSession.startDate, firstSession.endTime);
        const now = nowIST();
    ...
    const endDateTimestamp = istDayEnd(firstSession.endDate, firstSession.endTime);
    const now = nowIST();
